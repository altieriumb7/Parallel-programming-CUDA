...
bool parallel_sort_2(unsigned long *dev_a,
    ...
    if (config.total_blocks == 1)
    {
        ...
        */
        if ((config.required_shared_memory <= config.max_shared_memory_per_block) && shared_memory == true)
        {
            ...
            sort_kernel_2<<<config.gridSize, config.blockSize, config.required_shared_memory>>>(dev_a, N, config.partition_size, config.total_threads,true);
        }
        else
        {
            sort_kernel_2<<<config.gridSize, config.blockSize>>>(dev_a, N, config.partition_size, config.total_threads,false);
        }
    }
    else
    {
        ...
        */
        ...
        if ((config.required_shared_memory <= config.max_shared_memory_per_block) && shared_memory == true)
        {
            ...
            // Launch the kernel with the correct shared memory size
            radix_sort_kernel<<<config.gridSize, config.blockSize, config.required_shared_memory>>>(dev_a, N, config.partition_size, config.total_threads,true);
        }
        else
        {
            radix_sort_kernel<<<config.gridSize, config.blockSize>>>(dev_a, N, config.partition_size, config.total_threads,false);
        }
        ...
        */
        if (blocks_involved_in_merging > 1)
        {
            if ((required_shared_memory_data <= config.max_shared_memory_per_block) && shared_memory == true)
            {
                ...
                // Launch the kernel with the correct shared memory size
                merge_blocks_kernel_combined<<<1, blocks_involved_in_merging / 2, required_shared_memory_data>>>(dev_a, N, config, blocks_involved_in_merging / 2,true);
            }
            else
            {
                merge_blocks_kernel_combined<<<1, blocks_involved_in_merging / 2>>>(dev_a, N, config, blocks_involved_in_merging / 2,false);
            }
        }
    }
...
bool parallel_sort(unsigned long *dev_a,
                   const unsigned long long N,
                   ParallelSortConfig config,
                   const size_t size_blocks,
                   const unsigned blocks_involved_in_merging,
    ...
    if (config.total_blocks == 1)
    {
        ...
        */
        if ((config.required_shared_memory <= config.max_shared_memory_per_block) && shared_memory == true)
        {
            used_shared_memory = true;
            // Launch the kernel with the correct shared memory size
            sort_kernel<<<config.gridSize, config.blockSize, config.required_shared_memory>>>(dev_a, N, config.partition_size, config.total_threads,true);
        }
        else
        {
            sort_kernel<<<config.gridSize, config.blockSize>>>(dev_a, N, config.partition_size, config.total_threads,false);
        }
    }
    else
    {
...
__global__ void sort_kernel(unsigned long *data, const unsigned long long N, unsigned long long offset, const unsigned long total_threads, bool useSharedMemory)
{
    ...
    if (tid != 0)
    {
        ...
        else
        {
            ...
            {
                /*
                    This if-else is useful if there are more threads than needed:
                        - Ensures that no necessary thread remains idle
                */
                if ((N - old_offset) > 0)
                {
                    /*
                        - Compute the old offset by computing how much offset the precedent threads have
    ...
    /*
        This if-else is useful if there are more threads than needed:
            - It ensures that no necessary thread remains idle
    */
    if ((N - old_offset) > 0)
    {
        ...
        /*
            ...
            Compute the number of merges needed in the merge sort
        */
        ...
        if (useSharedMemory)
        {
            ...
            {
                /*
                    - At the first level, mid is equal to the end of the portion sorted by the thread since during the merging phase,
                ...
                */
                ...
                /*
                    - Threads_to_merge = 2^(level_merge) - Useful to exclude no necessary threads in the successive level
                ...
                */
                ...
                if ((tid % threads_to_merge) == 0)
                {
                    ...
                    */
                    ...
                    /*
                        - Merge sort expects mid as the maximum value of the first list
                        - Mid is equal to right for the next level_merge
                    */
                    mid = right;
                }
...
__global__ void sort_kernel_2(unsigned long *data, const unsigned long long N, unsigned long long offset, const unsigned long total_threads, bool useSharedMemory)
{
    const unsigned long tid = blockIdx.x * blockDim.x + threadIdx.x;
                ...
                */
                ...
                if ((tid % threads_to_merge) == 0)
                {
                    ...
                    */
                    ...
                    radix_sort_gpu(data + start, right);
